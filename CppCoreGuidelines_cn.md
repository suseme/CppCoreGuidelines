# <a name="main"></a>C++ Core Guidelines / C++核心指南
...
这是一份持续改进的活文档。 如果它是一个开源（代码）项目，这将是0.8版本。 复制、使用、修改和从本项目创建衍生作品的许可是根据MIT风格许可证授权的。 参与本项目需要同意贡献者许可。有关详细信息，请参见随附的[许可证](https://github.com/isocpp/CppCoreGuidelines/blob/master/LICENSE)文件。 我们向“友好用户”提供此项目，供他们使用、复制、修改和派生，希望得到建设性的反馈。
...
我们非常欢迎评论和改进建议。 我们计划随着我们对语言和可用库集的理解的提高以及语言和库集自身的改进，对这份文档进行修改和扩展。 在评论时，请参考[引言](#s-introduction)，它概述了我们的目标和一般方法。 贡献者名单在[这里](#ss-ack)。

Problems:
已知问题：

* The sets of rules have not been completely checked for completeness, consistency, or enforceability.
* 规则集尚未完全检查以确保完整性、一致性或可执行性。
* Triple question marks (???) mark known missing information.
* 三个问号（???）标记已知缺失的信息。
* Update reference sections; many pre-C++11 sources are too old.
* 更新参考资料部分；许多C++11之前的来源已经过时。
* For a more-or-less up-to-date to-do list see: [To-do: Unclassified proto-rules](#s-unclassified).
* 要查看一个大致最新的待办事项列表，请查看：[待办事项：未分类的原型规则](#s-unclassified)。

You can [read an explanation of the scope and structure of this Guide](#s-abstract) or just jump straight in:
你可以[阅读本指南范围和结构的解释](#s-abstract)，也可以直接进入

* [In: Introduction / 介绍](#s-introduction)
* [P: Philosophy / 理念](#s-philosophy)
...
* [T: Templates and generic programming / 模板与泛型编程](#s-templates)
* [CPL: C-style programming / C风格编程](#s-cpl)
* [SF: Source files / 源文件](#s-source)
* [SL: The Standard Library / 标准库](#sl-the-standard-library)

Supporting sections:
支持部分：

* [A: Architectural ideas / 架构思想](#s-a)
* [NR: Non-Rules and myths / 非规则和误区](#s-not)
* [RF: References / 参考文献](#s-references)
* [Pro: Profiles / 配置文件](#s-profile)
* [GSL: Guidelines support library / 支持库指南](#gsl-guidelines-support-library)
* [NL: Naming and layout suggestions / 命名和布局建议](#s-naming)
* [FAQ: Answers to frequently asked questions / 常见问题解答](#s-faq)
* [Appendix A: Libraries / 附录A: 库](#s-libraries)
* [Appendix B: Modernizing code / 附录B: 代码现代化](#s-modernizing)
* [Appendix C: Discussion / 附录C: 讨论](#s-discussion)
* [Appendix D: Supporting tools / 附录D: 支持工具](#s-tools)
* [Glossary / 词汇表](#s-glossary)
* [To-do: Unclassified proto-rules / 未分类的原型规则](#s-unclassified)

You can sample rules for specific language features:
您可以选取特定语言功能的规则：

* assignment / 赋值:
[regular types / 正则类型](#rc-regular) --
[prefer initialization / 预初始化](#rc-initialize) --
[copy / 拷贝](#rc-copy-semantic) --
[move / 移动](#rc-move-semantic) --
[other operations / 其他操作](#rc-matched) --
[default / 默认](#rc-eqdefault)
* `class` / 类:
[data / 数据](#rc-org) --
[invariant / 不变量](#rc-struct) --
[members / 成员](#rc-member) --
[helpers](#rc-helper) --
[concrete types / 具体类型](#ss-concrete) --
[ctors, =, and dtors / 构造，=，析构](#s-ctor) --
[hierarchy / 继承](#ss-hier) --
[operators / 操作符](#ss-overload)
* `concept` / 概念:
[rules / 规则](#ss-concepts) --
[in generic programming / 泛型编程](#rt-raise) --
[template arguments / 模板参数](#rt-concepts) --
[semantics / 语义学](#rt-low)
* constructor / 构造函数:
[invariant / 不变量](#rc-struct) --
...
[`explicit` / 显示](#rc-explicit) --
[delegating / 委托](#rc-delegating) --
[`virtual` / 虚](#rc-ctor-virtual)
* derived `class` / 派生类:
[when to use / 何时使用](#rh-domain) --
[as interface / 作为接口](#rh-abstract) --
...
* destructor / 析构函数:
[and constructors / 和构造函数](#rc-matched) --
[when needed? / 何时需要？](#rc-dtor) --
[must not fail / 不可失败](#rc-dtor-fail)
* exception / 异常:
[errors / 错误](#s-errors) --
[`throw` / 抛出](#re-throw) --
[for errors only / 只针对错误](#re-errors) --
[`noexcept`](#re-noexcept) --
[minimize `try` / 最小化`try`](#re-catch) --
[what if no exceptions? / 如果没有异常会是什么？](#re-no-throw-codes)
* `for`:
...
* `inline` / 内联:
[small functions / 小函数](#rf-inline) --
[in headers / 在头文件中](#rs-inline)
* initialization / 初始化:
[always / 总是](#res-always) --
[prefer `{}`](#res-list) --
[lambdas](#res-lambda-init) --
[default member initializers / 默认成员初始化](#rc-in-class-initializer) --
[class members / 类成员](#rc-initialize) --
[factory functions / 工厂方法](#rc-factory)
* lambda expression / lamba表达式:
[when to use / 何时采用](#ss-lambdas)
* operator / 操作符:
[conventional / 通常](#ro-conventional) --
[avoid conversion operators / 避免转换操作符](#ro-conversion) --
[and lambdas / 和匿名函数](#ro-lambda)
...
* assertion / 断言: ???
* error / 错误: ???
* exception: exception guarantee / 异常：异常保证 (???)
* failure / 失效: ???
* invariant / 不变: ???
* leak / 泄漏: ???
* library / 库: ???
...
# <a name="s-abstract"></a>Abstract / 摘要
...
这份文档是一套关于如何良好使用 C++的指南。
此文档的目的在于帮助人们有效地使用现代 C++。所谓“现代 C++”，我们指的是对 ISO C++标准（当前为 C++20，但我们几乎所有的建议也适用于 C++17、C++14 和 C++11）的有效运用。
换句话说，假设你现在就可以开始，那么你希望自己的代码在 5 年后、10 年后是什么样子？
...
这些指南主要关注相对高层级的问题，比如接口、资源管理、内存管理和并发。
此类规则会影响应用程序架构和库设计。
遵循这些规则将产生静态类型安全、无资源泄漏并且能捕捉到比当今代码中更多编程逻辑错误的代码。
而且它的运行速度会很快——你能够把事情做好。
...
我们不太关注低层级的问题，比如命名约定和缩进风格。
然而，任何能够帮助程序员的主题都没有超出范围。
...
我们最初的这套规则强调安全性（各种形式的）和简洁性。
它们很可能过于严格。
我们预计不得不引入更多的例外情况，以更好地适应现实世界的需求。
我们也需要更多的规则。
...
你会发现有些规则与你的预期相悖，甚至与你的经验相悖。
如果我们没有建议你以任何方式改变你的编码风格，那我们就失败了！
请尝试验证或反驳这些规则！
特别是，我们非常希望我们的一些规则能有测量数据或更好的示例作为支持。
...
你会发现有些规则是显而易见的，甚至是微不足道的。
请记住，指南的一个目的是帮助经验较少、来自不同背景或使用不同语言的人快速上手。
...
许多规则旨在得到分析工具的支持。
违反规则的情况将被标记，并带有相关规则的参考（或链接）。
我们不期望你在尝试编写代码之前记住所有的规则。
思考这些指南的一种方式是，将其视为碰巧可供人类阅读的工具规范。

The rules are meant for gradual introduction into a code base.
We plan to build tools for that and hope others will too.

这些规则旨在逐步引入到代码库中。
我们计划为此构建工具，并希望其他人也能这样做。

Comments and suggestions for improvements are most welcome.
We plan to modify and extend this document as our understanding improves and the language and the set of available libraries improve.

非常欢迎提出评论和改进建议。
我们计划随着我们理解的提高以及语言和可用库集的改进来修改和扩展此文档。

# <a name="s-introduction"></a>In: Introduction / 介绍
...
这是一套关于现代 C++（当前为 C++20 和 C++17）的核心指南，同时考虑了可能的未来增强功能和 ISO 技术规范（TSs）。
其目的是帮助 C++程序员编写更简单、更高效、更易于维护的代码。
...
* [In.aims: Aims / 目标](#ss-aims)
* [In.not: Non-aims / 非目标](#ss-non)
* [In.force: Enforcement / 执行](#ss-force)
* [In.struct: The structure of this document / 本文档的结构](#ss-struct)
...
所有 C++ 程序员。这包括[可能考虑C的程序员](#s-cpl)。

## <a name="ss-aims"></a>In.aims: Aims / 目标
...
这份文档的目的是帮助开发人员采用现代C++（当前为 C++20 和 C++17），并在代码库中实现更统一的风格。
...
我们并不幻想这些规则中的每一条都能有效地应用于每一个代码库。升级旧系统是很困难的。然而，我们确实相信，使用规则的程序比不使用规则的程序更不容易出错，也更易于维护。通常，规则也会带来更快/更容易的初始开发。
就我们所知，这些规则所产生的代码性能与更旧、更传统的技术相当或更好；它们旨在遵循零开销原则（“你不使用的东西，你无需为之付出代价”或者“当你恰当地使用抽象机制时，你至少能获得与使用低级语言结构手工编码相同的性能”）。
将这些规则视为新代码的理想标准，在处理旧代码时可利用的机会，并尽可能地接近这些理想标准。
请记住：

### <a name="r0"></a>In.0: Don't panic! / 别惊慌！
...
这些指南是根据“超集的子集”原则设计的（[Stroustrup05](#Stroustrup05)）。
它们并非简单地定义一个要使用的 C++子集（出于可靠性、安全性、性能或其他原因）。
相反，它们强烈推荐使用一些简单的“扩展”（[库组件](#gsl-guidelines-support-library)）
这使得 C++中最容易出错的特性变得多余，因此可以（在我们的规则集中）规避。
...
这些规则强调静态类型安全和资源安全。
出于这个原因，它们强调范围检查的可能性，避免解引用 `nullptr`，避免悬空指针，以及系统地使用异常（通过 RAII）。
部分是为了实现这一点，部分是为了将必要的复杂性隐藏在明确指定的接口后面以减少晦涩代码作为错误源，这些规则还强调简单性。
...
许多规则是规定性的。
我们对于那些只是说“不要那样做！”而不提供替代方案的规则感到不舒服。
这样的一个结果是，有些规则只能通过启发式方法得到支持，而不是精确和机械可验证的检查。
其他规则则阐述了一般原则。对于这些更一般的规则，更详细和具体的规则提供了部分检查。
...
这些指南涉及 C++的核心及其使用。
我们预计大多数大型组织、特定应用领域，甚至大型项目都需要进一步的规则，可能是进一步的限制，以及进一步的库支持。
例如，硬实时程序员通常不能自由使用自由存储（动态内存），并且在库的选择上会受到限制。
我们鼓励制定这样更具体的规则作为这些核心指南的附录。
构建您理想的小型基础库并使用它，而不是将您的编程水平降低到美化的汇编代码。

The rules are designed to allow [gradual adoption](#s-modernizing).

这些规则旨在允许[逐步采用](#S-modernizing)。
...
有些规则旨在增加各种形式的安全性，而另一些规则旨在降低事故的可能性，许多规则两者兼具。
旨在预防事故的指南常常禁止完全合法的 C++。
然而，当有两种表达一个想法的方式，并且其中一种已被证明是常见的错误源，而另一种不是，我们会尝试引导程序员采用后者。

## <a name="ss-non"></a>In.not: Non-aims / 非目标
...
这些规则并非旨在做到最小化或互不相关。
特别是，一般规则可能简单，但无法强制执行。
而且，通常很难理解一般规则的含义。
更具体的规则往往更容易理解和执行，但如果没有一般规则，它们就只是一长串特殊情况。
我们提供旨在帮助新手的规则以及支持专家使用的规则。
有些规则可以完全强制执行，但其他规则基于尝试。
...
这些规则并非像一本书那样要按顺序阅读。
您可以使用链接浏览它们。
然而，它们的主要预期用途是以开发为工具的目标。
也就是说，会有工具来查找违规行为，并且工具会返回违规规则的链接。
然后，规则会提供原因、违规可能导致的潜在后果的示例以及建议的补救措施。
...
这些指南并非旨在替代 C++ 的教程式讲解。
如果您需要针对特定经验水平的教程，请查看[参考文献](#s-references)。
...
这不是关于如何将旧的 C++代码转换为更现代代码的指南。
它旨在以具体的方式为新代码阐明思路。
但是，请查看[现代化章节](#s-modernizing)以获取一些可能的现代化/改进/升级方法。
重要的是，这些规则支持逐步采用：一次性完全转换大型代码库通常是不可行的。
...
这些指南并非是要在每个语言技术细节上都完整或精确。
关于语言定义问题的最终定论，包括一般规则的每个例外和每个特性，请参考 ISO C++ 标准。
...
这些规则并非旨在迫使您使用 C++的贫乏子集编写代码。
它们*绝对*不是要定义一个类似 Java 的 C++子集。
它们不是要定义一种单一的“真正的 C++”语言。
我们重视表达力和无妥协的性能。
...
这些规则并非价值中立。
它们旨在使代码比大多数现有的 C++代码更简单、更正确/更安全，且不损失性能。
它们旨在禁止与错误、虚假复杂性和低性能相关的完全有效的 C++代码。
...
这些规则并非精确到一个人（或机器）无需思考就能遵循的程度。
执行部分试图做到这一点，但我们宁愿让规则或定义有点模糊
并允许解释，而不是精确地指定错误的内容。
有时，精确性只有随着时间和经验才能到来。
设计（目前）还不是一种数学形式。
...
这些规则并不完美。
一条规则可能会因为禁止在特定情况下有用的东西而造成损害。
一条规则可能会因为未能禁止在特定情况下导致严重错误的东西而造成损害。
一条规则可能会因为模糊、含糊、无法执行或允许对问题的所有解决方案而造成很大的损害。
完全满足“不造成损害”的标准是不可能的。
相反，我们的目标不那么雄心勃勃：“为大多数程序员带来最大的好处”；
如果您无法接受某条规则，请反对它，忽略它，但不要削弱它直到它变得毫无意义。
同时，提出改进建议。

## <a name="ss-force"></a>In.force: Enforcement / 执行
...
对于大型代码库，没有执行机制的规则难以管理。
对所有规则的执行仅对于少量较弱的规则集或特定的用户群体才有可能。

* But we want lots of rules, and we want rules that everybody can use.
* 但我们想要很多规则，并且我们希望每个人都能使用这些规则。
...
我们希望指南能帮助很多人，使代码更加统一，并强烈鼓励人们将其代码现代化。
我们希望鼓励最佳实践，而不是将一切留给个人选择和管理压力。
理想情况是使用所有规则；这会带来最大的益处。
...
这加起来就有不少困境。
我们尝试使用工具来解决这些问题。
每条规则都有一个**执行**部分，列出了执行的想法。
执行可以通过代码审查、静态分析、编译器或运行时检查来完成。
只要可能，我们更倾向于“机械”检查（人类速度慢、不准确且容易感到厌烦）和静态检查。
只有在没有其他选择的情况下，才很少建议进行运行时检查；我们不想引入“分布式膨胀”。
在适当的地方，我们在**执行**部分中用相关规则组（称为“配置文件”）的名称标记规则。
一条规则可以是几个配置文件的一部分，也可以不属于任何配置文件。
一开始，我们有几个与常见需求（愿望、理想）相对应的配置文件：

* **type**: No type violations (reinterpreting a `T` as a `U` through casts, unions, or varargs)
* **类型**：无类型违规（通过强制转换、联合或可变参数将 `T` 重新解释为 `U`）
...
* **生存期**：无泄漏（未能 `delete` 或多次 `delete`）且无对无效对象的访问（解引用 `nullptr`，使用悬空引用）。
...
这些配置文件旨在供工具使用，但也有助于人类读者。
我们在**执行**部分中的评论不限于我们知道如何执行的内容；有些评论仅仅是愿望，可能会启发一些工具构建者。

Tools that implement these rules shall respect the following syntax to explicitly suppress a rule:

实现这些规则的工具应遵循以下语法来显式抑制规则：

    [[gsl::suppress("tag")]]

and optionally with a message (following usual C++11 standard attribute syntax):

并且可以选择带有消息（遵循常见的 C++11 标准属性语法）：
...
* `"tag"` 是一个字符串字面量，带有执行规则出现的项目的锚名称（例如，对于 [C.134](#rh-public) ，它是 "rh-public"）、配置文件规则组的名称（“类型”、“边界”或“生存期”）
或配置文件中的特定规则（[type.4](#pro-type-cstylecast) 或 [bounds.2](#pro-bounds-arrayindex)）。任何不属于这些的文本都应被拒绝。

* `"message"` is a string literal

* `"message"` 是一个字符串字面量

## <a name="ss-struct"></a>In.struct: The structure of this document / 本文档的结构

Each rule (guideline, suggestion) can have several parts:

每条规则（指导方针、建议）可以有几个部分：
...
有些规则很难机械地检查，但它们都满足最低标准，即专家程序员能够毫不费力地发现许多违规行为。
我们希望“机械”工具会随着时间的推移改进，以接近这样的专家程序员所注意到的情况。
此外，我们假设规则会随着时间的推移进行完善，使其更精确和可检查。
...
规则旨在简单明了，而非精心措辞以提及每个替代方案和特殊情况。
此类信息可在**替代方案**段落和[讨论](#s-discussion)部分中找到。
如果您不理解某条规则或不同意它，请访问其**讨论部分**。
如果您觉得某个讨论缺失或不完整，请提交一个[问题](https://github.com/isocpp/CppCoreGuidelines/issues)
解释您的关注点，并可能提交相应的 PR。
...
* 示例并非旨在达到生产质量或涵盖所有教程维度。
  例如，许多示例是语言技术方面的，并使用像 `f`、`base` 和 `x` 这样的名称。
...
这不是一本语言手册。
它旨在提供帮助，而非完整、在技术细节上完全准确或作为现有代码的指南。
推荐的信息来源可以在[参考文献](#s-references)中找到。
...
* [CP: Concurrency and parallelism / 并发和并行](#s-concurrency)
* [E: Error handling / 错误处理](#s-errors)
* [Con: Constants and immutability / 常量和不可变性](#s-const)
...
* [A: Architectural ideas / 架构理念](#s-a)
* [NR: Non-Rules and myths / 非规则和误区](#s-not)
* [RF: References / 参考文献](#s-references)
* [Pro: Profiles / 配置文件](#s-profile)
* [GSL: Guidelines support library / 指南支持库](#gsl-guidelines-support-library)
* [NL: Naming and layout suggestions / 命名和布局建议](#s-naming)
* [FAQ: Answers to frequently asked questions / 常见问题解答](#s-faq)
* [Appendix A: Libraries / 附录 A: 库](#s-libraries)
* [Appendix B: Modernizing code / 附录 B: 代码现代化](#s-modernizing)
* [Appendix C: Discussion / 附录 C: 讨论](#s-discussion)
* [Appendix D: Supporting tools / 附录 D: 支持工具](#s-tools)
* [Glossary / 词汇表](#s-glossary)
* [To-do: Unclassified proto-rules / 未分类的原型规则](#s-unclassified)
...
每个章节（例如，“P”代表“理念”）和每个子章节（例如，“C.hier”代表“类层次结构（OOP）”）都有一个缩写，以便于搜索和引用。
主要部分的缩写也用于规则编号（例如，“C.11”代表“使具体类型规则化”）。

# <a name="s-philosophy"></a>P: Philosophy / 理念
...
* [P.2: Write in ISO Standard C++ / 用 ISO 标准 C++ 编写](#rp-cplusplus)
* [P.3: Express intent / 表达意图](#rp-what)
* [P.4: Ideally, a program should be statically type safe / 理想情况下，一个程序应该是静态类型安全的](#rp-typesafe)
* [P.5: Prefer compile-time checking to run-time checking / 优先选择编译时检查而非运行时检查](#rp-compile-time)
* [P.6: What cannot be checked at compile time should be checkable at run time / 在编译时无法检查的内容应该在运行时可检查](#rp-run-time)
* [P.7: Catch run-time errors early / 尽早捕获运行时错误](#rp-early)
* [P.8: Don't leak any resources / 不要泄漏任何资源](#rp-leak)
* [P.9: Don't waste time or space / 不要浪费时间或空间](#rp-waste)
* [P.10: Prefer immutable data to mutable data / 优先选择不可变数据而非可变数据](#rp-mutable)
* [P.11: Encapsulate messy constructs, rather than spreading through the code / 将混乱的结构进行封装，而不是在代码中到处散布](#rp-library)
* [P.12: Use supporting tools as appropriate / 适当地使用辅助工具](#rp-tools)
* [P.13: Use support libraries as appropriate / 适当地使用支持库](#rp-lib)
...
哲学规则通常无法通过机械方式进行检验。
然而，反映这些哲学主题的个别规则是可以的。
没有哲学基础，更具体/特定/可检验的规则就缺乏合理性。

### <a name="rp-direct"></a>P.1: Express ideas directly in code / 直接在代码中表达想法
...
编译器不会读取注释（或设计文档），很多程序员也不会（始终如此）。 
代码中所表达的内容具有明确的语义，并且（原则上）可以由编译器和其他工具进行检查。 
...
`month` 的第一个声明明确说明了返回一个 `Month` 类型，并且不会修改 `Date` 对象的状态。 第二个版本让读者去猜测，并且为未捕获的错误增加了更多的可能性。 

##### Example, bad / 示例（不好的）

This loop is a restricted form of `std::find`:

这个循环是 `std::find` 的一种受限形式： 
...
一个设计良好的库能比直接使用语言特性更好地表达意图（要做什么，而不仅仅是如何去做）。
...
一个 C++ 程序员应该了解标准库的基础知识，并在适当的地方使用它。
任何程序员都应该了解正在进行的项目的基础库的基础知识，并适当地使用它们。
任何使用这些指南的程序员都应该了解[指南支持库](#gsl-guidelines-support-library)，并适当地使用它。
...
更好的方法是明确这个双精度数的含义（是新的速度还是旧速度的增量？）以及所使用的单位： 
...
我们本可以接受一个简单的（无单位的）`double` 作为增量，但那样容易出错。 如果我们既想要绝对速度又想要增量，我们就会定义一个 `Delta` 类型。 
...
* 始终一致地使用 `const`（检查成员函数是否修改其对象；检查函数是否修改通过指针或引用传递的参数）
* flag uses of casts (casts neuter the type system)
* 标记强制类型转换的使用（强制类型转换会削弱类型系统）
* detect code that mimics the standard library (hard)
* 检测模仿标准库的代码（困难）

### <a name="rp-cplusplus"></a>P.2: Write in ISO Standard C++ / 用 ISO 标准 C++ 编写

##### Reason / 原因

This is a set of guidelines for writing ISO Standard C++.

这是一套关于编写 ISO 标准 C++ 的指南。 
...
在某些环境中，扩展是必要的，例如访问系统资源。
在这种情况下，本地化必要扩展的使用，并使用非核心编码指南来控制其使用。如果可能，构建封装扩展的接口，以便在不支持这些扩展的系统上可以关闭或不编译它们。
...
扩展通常没有严格定义的语义。即使是常见的、由多个编译器实现的扩展，也可能由于*没有*严格的标准定义而具有略微不同的行为和边缘情况行为。过度使用任何此类扩展都会影响预期的可移植性。
...
使用有效的 ISO C++ 并不能保证可移植性（更不用说正确性了）。
避免依赖未定义的行为（例如，[未定义的求值顺序](#res-order)），
并注意具有实现定义含义的结构（例如，`sizeof(int)`）。
...
在某些环境中，限制使用标准 C++ 语言或库特性是必要的，例如，按照飞机控制软件标准的要求避免动态内存分配。 在这种情况下，使用针对特定环境定制的这些编码指南的扩展来控制它们的（禁止）使用。 

##### Enforcement / 执行

Use an up-to-date C++ compiler (currently C++20 or C++17) with a set of options that do not accept extensions.

使用最新的 C++ 编译器（当前为 C++20 或 C++17），并设置一组不接受扩展的选项。 

### <a name="rp-what"></a>P.3: Express intent / 表达意图 

##### Reason / 原因

Unless the intent of some code is stated (e.g., in names or comments), it is impossible to tell whether the code does what it is supposed to do.

除非某些代码的意图已被说明（例如，在名称或注释中），否则无法判断该代码是否在做它应该做的事情。
...
在这里，“只是”遍历 `v` 的元素这一意图并未得到表达。索引的实现细节被暴露了（因此可能会被误用），并且 `i` 的作用域超出了循环，这可能是有意的，也可能不是。仅从这段代码，读者无法知晓。 
...
现在，这里没有明确提及迭代机制，并且该循环是基于对 `const` 元素的引用进行操作的，以避免意外修改。如果需要进行修改，请明确说明：
...
有关 for 语句的更多详细信息，请参阅 [ES.71](#res-for-range) 。 有时更好的做法是使用一个已命名的算法。此示例使用了来自 Ranges TS 的 `for_each` ，因为它直接表达了意图： 
...
* [指南支持库](#gsl-guidelines-support-library)
...
如果两个 `int` 类型的值意在表示二维点的坐标，那就这样说明：
...
这里有很大的空间可以进行巧妙的设计和半自动的程序转换。

### <a name="rp-typesafe"></a>P.4: Ideally, a program should be statically type safe / 理想情况下，一个程序应该是静态类型安全的
...
### <a name="rp-compile-time"></a>P.5: Prefer compile-time checking to run-time checking / 优先选择编译时检查而非运行时检查
...
### <a name="rp-run-time"></a>P.6: What cannot be checked at compile time should be checkable at run time / 在编译时无法检查的内容应该在运行时可检查
...
### <a name="rp-early"></a>P.7: Catch run-time errors early / 尽早捕获运行时错误
...
### <a name="rp-leak"></a>P.8: Don't leak any resources / 不要泄漏任何资源
...
### <a name="rp-waste"></a>P.9: Don't waste time or space / 不要浪费时间或空间
...
### <a name="rp-mutable"></a>P.10: Prefer immutable data to mutable data / 优先选择不可变数据而非可变数据
...
### <a name="rp-library"></a>P.11: Encapsulate messy constructs, rather than spreading through the code / 将混乱的结构进行封装，而不是在代码中到处散布
...
### <a name="rp-tools"></a>P.12: Use supporting tools as appropriate / 适当地使用辅助工具
...
### <a name="rp-lib"></a>P.13: Use support libraries as appropriate / 适当地使用支持库
...
# <a name="s-interfaces"></a>I: Interfaces / 接口
...
* [I.1: Make interfaces explicit / 明确接口](#ri-explicit)
* [I.2: Avoid non-`const` global variables / 避免使用非 `const` 的全局变量](#ri-global)
* [I.3: Avoid singletons / 避免使用单例](#ri-singleton)
* [I.4: Make interfaces precisely and strongly typed / 使接口精确且强类型化](#ri-typed)
* [I.5: State preconditions (if any) / 说明前置条件（如果有的话）](#ri-pre)
* [I.6: Prefer `Expects()` for expressing preconditions / 优先使用 `Expects()` 来检查前置条件](#ri-expects)
* [I.7: State postconditions / 说明后置条件](#ri-post)
* [I.8: Prefer `Ensures()` for expressing postconditions / 优先使用 `Ensures()` 来检查后置条件](#ri-ensures)
* [I.9: If an interface is a template, document its parameters using concepts / 如果一个接口是模板，请使用概念来记录其参数](#ri-concepts)
* [I.10: Use exceptions to signal a failure to perform a required task / 使用异常来表示执行所需任务失败](#ri-except)
* [I.11: Never transfer ownership by a raw pointer (`T*`) or reference (`T&`) / 永远不要通过原始指针（`T*`）或引用（`T&`）转移所有权](#ri-raw)
* [I.12: Declare a pointer that must not be null as `not_null` / 将不得为空的指针声明为 `not_null`](#ri-nullptr)
* [I.13: Do not pass an array as a single pointer / 不要将数组作为单个指针传递](#ri-array)
* [I.22: Avoid complex initialization of global objects / 避免对全局对象进行复杂的初始化](#ri-global-init)
* [I.23: Keep the number of function arguments low / 保持较少的函数参数数量](#ri-nargs)
* [I.24: Avoid adjacent parameters that can be invoked by the same arguments in either order with different meaning / 避免相邻的参数可以通过相同的参数以不同的顺序调用而具有不同的含义](#ri-unrelated)
* [I.25: Prefer empty abstract classes as interfaces to class hierarchies / 优先使用空的抽象类作为接口，而非类层次结构](#ri-abstract)
* [I.26: If you want a cross-compiler ABI, use a C-style subset / 如果您想要一个交叉编译器 ABI，请使用 C 风格的子集](#ri-abi)
* [I.27: For stable library ABI, consider the Pimpl idiom / 对于稳定的库 ABI，考虑使用 Pimpl 惯用法](#ri-pimpl)
* [I.30: Encapsulate rule violations / 封装违反规则的情况](#ri-encapsulate)
...
### <a name="ri-explicit"></a>I.1: Make interfaces explicit / 明确接口
...
### <a name="ri-global"></a>I.2: Avoid non-`const` global variables / 避免使用非 `const` 的全局变量
...
### <a name="ri-singleton"></a>I.3: Avoid singletons / 避免使用单例
...
### <a name="ri-typed"></a>I.4: Make interfaces precisely and strongly typed / 使接口精确且强类型化
...
### <a name="ri-pre"></a>I.5: State preconditions (if any) / 说明前置条件（如果有的话）
...
### <a name="ri-expects"></a>I.6: Prefer `Expects()` for expressing preconditions / 优先使用 `Expects()` 来检查前置条件
...
### <a name="ri-post"></a>I.7: State postconditions / 说明后置条件
...
### <a name="ri-ensures"></a>I.8: Prefer `Ensures()` for expressing postconditions / 优先使用 `Ensures()` 来检查后置条件
...
### <a name="ri-concepts"></a>I.9: If an interface is a template, document its parameters using concepts / 如果一个接口是模板，请使用概念来记录其参数
...
### <a name="ri-except"></a>I.10: Use exceptions to signal a failure to perform a required task / 使用异常来表示执行所需任务失败
...
### <a name="ri-raw"></a>I.11: Never transfer ownership by a raw pointer (`T*`) or reference (`T&`) / 永远不要通过原始指针（`T*`）或引用（`T&`）转移所有权
...
### <a name="ri-nullptr"></a>I.12: Declare a pointer that must not be null as `not_null` / 将不得为空的指针声明为 `not_null`
...
### <a name="ri-array"></a>I.13: Do not pass an array as a single pointer / 不要将数组作为单个指针传递
...
### <a name="ri-global-init"></a>I.22: Avoid complex initialization of global objects / 避免对全局对象进行复杂的初始化
...
### <a name="ri-nargs"></a>I.23: Keep the number of function arguments low / 保持较少的函数参数数量
...
### <a name="ri-unrelated"></a>I.24: Avoid adjacent parameters that can be invoked by the same arguments in either order with different meaning / 避免相邻的参数可以通过相同的参数以不同的顺序调用而具有不同的含义
...
### <a name="ri-abstract"></a>I.25: Prefer empty abstract classes as interfaces to class hierarchies / 优先使用空的抽象类作为接口，而非类层次结构
...
### <a name="ri-abi"></a>I.26: If you want a cross-compiler ABI, use a C-style subset / 如果您想要一个交叉编译器 ABI，请使用 C 风格的子集
...
### <a name="ri-pimpl"></a>I.27: For stable library ABI, consider the Pimpl idiom / 对于稳定的库 ABI，考虑使用 Pimpl 惯用法
...
### <a name="ri-encapsulate"></a>I.30: Encapsulate rule violations / 封装违反规则的情况
...
# <a name="s-functions"></a>F: Functions / 函数
...
* [F.1: "Package" meaningful operations as carefully named functions / 将有意义的操作“打包”为精心命名的函数](#rf-package)
* [F.2: A function should perform a single logical operation / 一个函数应该执行单个逻辑操作](#rf-logical)
* [F.3: Keep functions short and simple / 保持函数简短和简单](#rf-single)
* [F.4: If a function might have to be evaluated at compile time, declare it `constexpr` / 如果一个函数可能需要在编译时进行计算，将其声明为 `constexpr`](#rf-constexpr)
* [F.5: If a function is very small and time-critical, declare it inline / 如果一个函数非常小且对时间关键，将其声明为内联函数](#rf-inline)
* [F.6: If your function must not throw, declare it `noexcept` / 如果您的函数一定不会抛出异常，将其声明为 `noexcept`](#rf-noexcept)
* [F.7: For general use, take `T*` or `T&` arguments rather than smart pointers / 对于一般用途，采用 `T*` 或 `T&` 参数而非智能指针](#rf-smart)
* [F.8: Prefer pure functions / 优先选择纯函数](#rf-pure)
* [F.9: Unused parameters should be unnamed / 未使用的参数应不命名](#rf-unused)
* [F.10: If an operation can be reused, give it a name / 如果一个操作可以重复使用，为其命名](#rf-name)
* [F.11: Use an unnamed lambda if you need a simple function object in one place only / 如果仅在一个地方需要一个简单的函数对象，请使用未命名的 lambda 表达式](#rf-lambda)

Parameter passing expression rules:

参数传递表达式规则

* [F.15: Prefer simple and conventional ways of passing information / 优先采用简单和常规的信息传递方式](#rf-conventional)
* [F.16: For "in" parameters, pass cheaply-copied types by value and others by reference to `const` / 对于“输入”参数，通过值传递可廉价拷贝的类型，其他的通过对 `const` 的引用传递](#rf-in)
* [F.17: For "in-out" parameters, pass by reference to non-`const` / 对于“输入-输出”参数，通过对非 `const` 的引用传递](#rf-inout)
* [F.18: For "will-move-from" parameters, pass by `X&&` and `std::move` the parameter / 对于“将移动自”参数，通过 `X&&` 传递并对参数使用 `std::move`](#rf-consume)
* [F.19: For "forward" parameters, pass by `TP&&` and only `std::forward` the parameter / 对于“转发”参数，通过 `TP&&` 传递并且仅对参数使用 `std::forward`](#rf-forward)
* [F.20: For "out" output values, prefer return values to output parameters / 对于“输出”输出值，优先使用返回值而不是输出参数](#rf-out)
* [F.21: To return multiple "out" values, prefer returning a struct / 要返回多个“输出”值，优先返回一个结构体](#rf-out-multi)
* [F.60: Prefer `T*` over `T&` when "no argument" is a valid option / 当“无参数”是有效选项时，优先使用 `T*` 而非 `T&`](#rf-ptr-ref)

Parameter passing semantic rules:

参数传递语义规则：

* [F.22: Use `T*` or `owner<T*>` to designate a single object / 使用 `T*` 或 `owner<T*>` 来指定单个对象](#rf-ptr)
* [F.23: Use a `not_null<T>` to indicate that "null" is not a valid value / 使用 `not_null<T>` 来表明“空值”不是一个有效的值](#rf-nullptr)
* [F.24: Use a `span<T>` or a `span_p<T>` to designate a half-open sequence / 使用 `span<T>` 或 `span_p<T>` 来指定半开序列](#rf-range)
* [F.25: Use a `zstring` or a `not_null<zstring>` to designate a C-style string / 使用 `zstring` 或 `not_null<zstring>` 来指定 C 风格字符串](#rf-zstring)
* [F.26: Use a `unique_ptr<T>` to transfer ownership where a pointer is needed / 在需要指针的地方使用 `unique_ptr<T>` 来转移所有权](#rf-unique_ptr)
* [F.27: Use a `shared_ptr<T>` to share ownership / 使用 `shared_ptr<T>` 来共享所有权](#rf-shared_ptr)

<a name="rf-value-return"></a>Value return semantic rules:

值返回语义规则：

* [F.42: Return a `T*` to indicate a position (only) / （仅）返回 `T*` 以指示位置](#rf-return-ptr)
* [F.43: Never (directly or indirectly) return a pointer or a reference to a local object / 永远不要（直接或间接）返回指向本地对象的指针或引用](#rf-dangle)
* [F.44: Return a `T&` when copy is undesirable and "returning no object" isn't needed / 当不希望拷贝且不需要“不返回对象”时，返回 `T&`](#rf-return-ref)
* [F.45: Don't return a `T&&` / 不要返回 `T&&`](#rf-return-ref-ref)
* [F.46: `int` is the return type for `main()` / `int` 是 `main()` 的返回类型](#rf-main)
* [F.47: Return `T&` from assignment operators / 从赋值运算符返回 `T&`](#rf-assignment-op)
* [F.48: Don't return `std::move(local)` / 不要返回 `std::move(local)`](#rf-return-move-local)
* [F.49: Don't return `const T` / 不要返回 `const T`](#rf-return-const)

Other function rules:

其他函数规则：

* [F.50: Use a lambda when a function won't do (to capture local variables, or to write a local function) / 当函数无法满足需求时（捕获局部变量或编写局部函数）使用 lambda 表达式](#rf-capture-vs-overload)
* [F.51: Where there is a choice, prefer default arguments over overloading / 在有选择的情况下，优先选择默认参数而不是重载](#rf-default-args)
* [F.52: Prefer capturing by reference in lambdas that will be used locally, including passed to algorithms / 在本地使用的 lambda 表达式中，包括传递给算法时，优先通过引用捕获](#rf-reference-capture)
* [F.53: Avoid capturing by reference in lambdas that will be used non-locally, including returned, stored on the heap, or passed to another thread / 在非本地使用的 lambda 表达式中，包括返回、存储在堆上或传递给另一个线程时，避免通过引用捕获](#rf-value-capture)
* [F.54: When writing a lambda that captures `this` or any class data member, don't use `[=]` default capture / 编写捕获 `this` 或任何类数据成员的 lambda 表达式时，不要使用 `[=]` 默认捕获](#rf-this-capture)
* [F.55: Don't use `va_arg` arguments / 不要使用 `va_arg` 参数](#f-varargs)
* [F.56: Avoid unnecessary condition nesting / 避免不必要的条件嵌套](#f-nesting)
...
## <a name="ss-fct-def"></a>F.def: Function definitions / 函数定义

A function definition is a function declaration that also specifies the function's implementation, the function body.

### <a name="rf-package"></a>F.1: "Package" meaningful operations as carefully named functions / 将有意义的操作“打包”为精心命名的函数
...
### <a name="rf-logical"></a>F.2: A function should perform a single logical operation / 一个函数应该执行单个逻辑操作
...
### <a name="rf-single"></a>F.3: Keep functions short and simple / 保持函数简短和简单
...
### <a name="rf-constexpr"></a>F.4: If a function might have to be evaluated at compile time, declare it `constexpr` / 如果一个函数可能需要在编译时进行计算，将其声明为 `constexpr`
...
### <a name="rf-inline"></a>F.5: If a function is very small and time-critical, declare it `inline` / 如果一个函数非常小且对时间关键，将其声明为内联函数
...
### <a name="rf-noexcept"></a>F.6: If your function must not throw, declare it `noexcept` / 如果您的函数一定不会抛出异常，将其声明为 `noexcept`
...
### <a name="rf-smart"></a>F.7: For general use, take `T*` or `T&` arguments rather than smart pointers / 对于一般用途，采用 `T*` 或 `T&` 参数而非智能指针
...
### <a name="rf-pure"></a>F.8: Prefer pure functions / 优先选择纯函数
...
### <a name="rf-unused"></a>F.9: Unused parameters should be unnamed / 未使用的参数应不命名
...
### <a name="rf-name"></a>F.10: If an operation can be reused, give it a name / 如果一个操作可以重复使用，为其命名
...
### <a name="rf-lambda"></a>F.11: Use an unnamed lambda if you need a simple function object in one place only / 如果仅在一个地方需要一个简单的函数对象，请使用未命名的 lambda 表达式
...
## <a name="ss-call"></a>F.call: Parameter passing / 参数传递

There are a variety of ways to pass parameters to a function and to return values.

### <a name="rf-conventional"></a>F.15: Prefer simple and conventional ways of passing information / 优先采用简单和常规的信息传递方式
...
### <a name="rf-in"></a>F.16: For "in" parameters, pass cheaply-copied types by value and others by reference to `const` / 对于“输入”参数，通过值传递可廉价拷贝的类型，其他的通过对 `const` 的引用传递
...
### <a name="rf-inout"></a>F.17: For "in-out" parameters, pass by reference to non-`const` / 对于“输入-输出”参数，通过对非 `const` 的引用传递
...
### <a name="rf-consume"></a>F.18: For "will-move-from" parameters, pass by `X&&` and `std::move` the parameter / 对于“将移动自”参数，通过 `X&&` 传递并对参数使用 `std::move`
...
### <a name="rf-forward"></a>F.19: For "forward" parameters, pass by `TP&&` and only `std::forward` the parameter / 对于“转发”参数，通过 `TP&&` 传递并且仅对参数使用 `std::forward`
...
### <a name="rf-out"></a>F.20: For "out" output values, prefer return values to output parameters / 对于“输出”输出值，优先使用返回值而不是输出参数

##### Reason / 原因

A return value is self-documenting, whereas an `&` could be either in-out or out-only and is liable to be misused.

返回值具有自说明性，而“&”既可以表示输入输出（in-out）也可以表示仅输出（out-only），并且容易被误用。

This includes large objects like standard containers that use implicit move operations for performance and to avoid explicit memory management.

这包括像标准容器这样的大型对象，它们使用隐式移动操作来提高性能并避免显式的内存管理。

If you have multiple values to return, [use a tuple](#rf-out-multi) or similar multi-member type.

如果你有多个值要返回，请[使用元组](#rf-out-multi)或类似的多成员类型。
...
一个由许多（单个移动成本低）元素组成的`struct`（结构体），其整体移动成本可能很高。
...
* 如果某个类型的移动成本很高（例如`array<BigTrivial>`），考虑在自由存储区上分配它并返回一个句柄（例如`unique_ptr`），或者将它传递到一个非`const`目标对象的引用中以进行填充（用作输出参数）。
...
* 要在内层循环中多次调用函数时重用具有容量的对象（例如`std::string`、`std::vector`）：[将其视为输入/输出参数并通过引用传递](#rf-out-multi)。

##### Example / 示例

Assuming that `Matrix` has move operations (possibly by keeping its elements in a `std::vector`):

假设`Matrix`具有移动操作（可能是通过将其元素保存在`std::vector`中）：
...
* 标记对在写入前未读取且可以廉价返回的类型的非`const`参数的引用；它们应该是“输出”返回值。

### <a name="rf-out-multi"></a>F.21: To return multiple "out" values, prefer returning a struct / 要返回多个“输出”值，优先返回一个结构体
...
返回值作为“仅输出”值具有自说明性。
请注意，C++确实有多个返回值，按照使用类似元组类型（`struct`、`array`、`tuple`等）的惯例，在调用点可能还会有结构化绑定（C++17）带来的额外便利。
如果可能，优先使用具名的`struct`。
否则，在可变参数模板中，`tuple`很有用。
...
C++98 的标准库在某些地方使用了这种风格，在一些函数中返回`pair`。 
例如，给定一个`set<string> my_set`，考虑：
...
在现代 C++中，具有有意义名称的`struct`更为常见。
例如，参见`ranges::min_max_result`、`from_chars_result`等。
...
有时候，我们需要将一个对象传递给一个函数以操作其状态。
在这种情况下，通过引用 [`T&`](#rf-inout) 传递对象通常是正确的技术。
将输入输出参数再次作为返回值显式地返回通常是没有必要的。
例如：
...
在这里，`s`和`in`都被用作输入输出参数。
我们通过（非`const`）引用传递`in`，以便能够操作它的状态。
我们传递`s`以避免重复分配内存。
通过重用（通过引用传递的）`s`，我们仅在需要扩展`s`的容量时才分配新内存。
这种技术有时被称为“调用者分配输出”模式，对于像`string`和`vector`这样需要进行自由存储区分配的类型特别有用。

To compare, if we passed out all values as return values, we would write something like this:

相比之下，如果我们将所有值都作为返回值传出，我们会写成这样：
...
对于对这条规则（F.21）的严格解读，这种例外情况其实并非真正的例外，因为它依赖于输入输出参数，而不是规则中提到的单纯的输出参数。
然而，我们更倾向于清晰明确，而不是隐晦微妙。
...
过于通用的`pair`和`tuple`只应在返回的值表示独立实体而非抽象概念时使用。
...
另一种选择是使用`optional<T>`或`expected<T, error_code>`，而不是`pair`或`tuple`。当使用得当时，这些类型比`pair<T, bool>`或`pair<T, error_code>`能传达更多关于成员含义的信息。
...
当要返回的对象是从拷贝成本很高的局部变量初始化时，显式的“移动”可能有助于避免拷贝：
...
请注意，这与[ES.56](#res-move)中的`return move(...)`反模式不同
...
* 输出参数应被返回值取代。输出参数是指函数写入、调用非`const`成员函数或作为非`const`传递的参数。
* `pair` or `tuple` return types should be replaced by `struct`, if possible.
  In variadic templates, `tuple` is often unavoidable.
* 如果可能，`pair`或`tuple`返回类型应被`struct`取代。在可变参数模板中，`tuple`常常是不可避免的。

### <a name="rf-ptr-ref"></a>F.60: Prefer `T*` over `T&` when "no argument" is a valid option / 当“无参数”是有效选项时，优先使用 `T*` 而非 `T&`
...
### <a name="rf-ptr"></a>F.22: Use `T*` or `owner<T*>` to designate a single object / 使用 `T*` 或 `owner<T*>` 来指定单个对象
...
### <a name="rf-nullptr"></a>F.23: Use a `not_null<T>` to indicate that "null" is not a valid value / 使用 `not_null<T>` 来表明“空值”不是一个有效的值
...
### <a name="rf-range"></a>F.24: Use a `span<T>` or a `span_p<T>` to designate a half-open sequence / 使用 `span<T>` 或 `span_p<T>` 来指定半开序列
...
### <a name="rf-zstring"></a>F.25: Use a `zstring` or a `not_null<zstring>` to designate a C-style string / 使用 `zstring` 或 `not_null<zstring>` 来指定 C 风格字符串
...
### <a name="rf-unique_ptr"></a>F.26: Use a `unique_ptr<T>` to transfer ownership where a pointer is needed / 在需要指针的地方使用 `unique_ptr<T>` 来转移所有权
...
### <a name="rf-shared_ptr"></a>F.27: Use a `shared_ptr<T>` to share ownership / 使用 `shared_ptr<T>` / 来共享所有权
...
### <a name="rf-return-ptr"></a>F.42: Return a `T*` to indicate a position (only) / （仅）返回 `T*` 以指示位置
...
### <a name="rf-dangle"></a>F.43: Never (directly or indirectly) return a pointer or a reference to a local object / 永远不要（直接或间接）返回指向本地对象的指针或引用
...
### <a name="rf-return-ref"></a>F.44: Return a `T&` when copy is undesirable and "returning no object" isn't needed / 当不希望拷贝且不需要“不返回对象”时，返回 `T&`
...
### <a name="rf-return-ref-ref"></a>F.45: Don't return a `T&&` / 不要返回 `T&&`
...
### <a name="rf-main"></a>F.46: `int` is the return type for `main()` / `int` 是 `main()` 的返回类型
...
### <a name="rf-assignment-op"></a>F.47: Return `T&` from assignment operators / 从赋值运算符返回 `T&`
...
### <a name="rf-return-move-local"></a>F.48: Don't `return std::move(local)` / 不要返回 `std::move(local)`
...
### <a name="rf-return-const"></a>F.49: Don't return `const T` / 不要返回 `const T`
...
### <a name="rf-capture-vs-overload"></a>F.50: Use a lambda when a function won't do (to capture local variables, or to write a local function) / 当函数无法满足需求时（捕获局部变量或编写局部函数）使用 lambda 表达式
...
### <a name="rf-default-args"></a>F.51: Where there is a choice, prefer default arguments over overloading / 在有选择的情况下，优先选择默认参数而不是重载
...
### <a name="rf-reference-capture"></a>F.52: Prefer capturing by reference in lambdas that will be used locally, including passed to algorithms / 在本地使用的 lambda 表达式中，包括传递给算法时，优先通过引用捕获
...
### <a name="rf-value-capture"></a>F.53: Avoid capturing by reference in lambdas that will be used non-locally, including returned, stored on the heap, or passed to another thread / 在非本地使用的 lambda 表达式中，包括返回、存储在堆上或传递给另一个线程时，避免通过引用捕获
...
### <a name="rf-this-capture"></a>F.54: When writing a lambda that captures `this` or any class data member, don't use `[=]` default capture / 编写捕获 `this` 或任何类数据成员的 lambda 表达式时，不要使用 `[=]` 默认捕获
...
### <a name="f-varargs"></a>F.55: Don't use `va_arg` arguments / 不要使用 `va_arg` 参数
...
### <a name="f-nesting"></a>F.56: Avoid unnecessary condition nesting / 避免不必要的条件嵌套
...
# <a name="s-class"></a>C: Classes and class hierarchies / 类和类层次结构
...
* [C.1: Organize related data into structures (`struct`s or `class`es) / 将相关数据组织到结构体（`struct` 或 `class`）中](#rc-org)
* [C.2: Use `class` if the class has an invariant; use `struct` if the data members can vary independently / 如果类具有不变量，则使用 `class`；如果数据成员可以独立变化，则使用 `struct`](#rc-struct)
* [C.3: Represent the distinction between an interface and an implementation using a class / 使用类来表示接口和实现之间的区别](#rc-interface)
* [C.4: Make a function a member only if it needs direct access to the representation of a class / 仅当函数需要直接访问类的表示时，才将其定义为成员函数](#rc-member)
* [C.5: Place helper functions in the same namespace as the class they support / 将辅助函数放在与它们所支持的类相同的命名空间中](#rc-helper)
* [C.7: Don't define a class or enum and declare a variable of its type in the same statement / 不要在同一条语句中定义类或枚举并声明其类型的变量](#rc-standalone)
* [C.8: Use `class` rather than `struct` if any member is non-public / 如果任何成员是非公共的，则使用 `class` 而不是 `struct`](#rc-class)
* [C.9: Minimize exposure of members / 尽量减少成员的暴露](#rc-private)

Subsections:

* [C.concrete: Concrete types / 具体类型](#ss-concrete)
* [C.ctor: Constructors, assignments, and destructors / 构造函数、赋值和析构函数](#s-ctor)
* [C.con: Containers and other resource handles / 容器和其他资源句柄](#ss-containers)
* [C.lambdas: Function objects and lambdas / 函数对象和 lambda 表达式](#ss-lambdas)
* [C.hier: Class hierarchies (OOP) / 类层次结构（面向对象编程）](#ss-hier)
* [C.over: Overloading and overloaded operators / 重载和重载运算符](#ss-overload)
* [C.union: Unions / 联合类型](#ss-union)

### <a name="rc-org"></a>C.1: Organize related data into structures (`struct`s or `class`es) / 将相关数据组织到结构体（`struct` 或 `class`）中
...
### <a name="rc-struct"></a>C.2: Use `class` if the class has an invariant; use `struct` if the data members can vary independently / 如果类具有不变量，则使用 `class`；如果数据成员可以独立变化，则使用 `struct`
...
### <a name="rc-member"></a>C.4: Make a function a member only if it needs direct access to the representation of a class / 仅当函数需要直接访问类的表示时，才将其定义为成员函数
...
### <a name="rc-helper"></a>C.5: Place helper functions in the same namespace as the class they support / 将辅助函数放在与它们所支持的类相同的命名空间中
...
### <a name="rc-standalone"></a>C.7: Don't define a class or enum and declare a variable of its type in the same statement / 不要在同一条语句中定义类或枚举并声明其类型的变量
...
### <a name="rc-class"></a>C.8: Use `class` rather than `struct` if any member is non-public / 如果任何成员是非公共的，则使用 `class` 而不是 `struct`
...
### <a name="rc-private"></a>C.9: Minimize exposure of members / 尽量减少成员的暴露
...
## <a name="ss-concrete"></a>C.concrete: Concrete types / 具体类型

Concrete type rule summary:

* [C.10: Prefer concrete types over class hierarchies / 相比于类层次结构，优先选择具体类型](#rc-concrete)
* [C.11: Make concrete types regular / 使具体类型规范](#rc-regular)
* [C.12: Don't make data members `const` or references in a copyable or movable type / 在可拷贝或可移动的类型中，不要将数据成员设置为 `const` 或引用](#rc-constref)


### <a name="rc-concrete"></a>C.10: Prefer concrete types over class hierarchies / 相比于类层次结构，优先选择具体类型
...
### <a name="rc-regular"></a>C.11: Make concrete types regular / 使具体类型规范
...
### <a name="rc-constref"></a>C.12: Don't make data members `const` or references in a copyable or movable type / 在可拷贝或可移动的类型中，不要将数据成员设置为 `const` 或引用
...
## <a name="s-ctor"></a>C.ctor: Constructors, assignments, and destructors / 构造函数、赋值和析构函数
...
* [C.20: If you can avoid defining any default operations, do / 如果可以避免定义任何默认操作，那就避免](#rc-zero)
* [C.21: If you define or `=delete` any copy, move, or destructor function, define or `=delete` them all / 如果定义或 `=delete` 任何拷贝、移动或析构函数，那么全部定义或 `=delete` 它们](#rc-five)
* [C.22: Make default operations consistent / 使默认操作保持一致](#rc-matched)

Destructor rules:

析构函数规则：

* [C.30: Define a destructor if a class needs an explicit action at object destruction / 如果一个类在对象销毁时需要明确的操作，则定义析构函数](#rc-dtor)
* [C.31: All resources acquired by a class must be released by the class's destructor / 一个类获取的所有资源都必须在该类的析构函数中释放](#rc-dtor-release)
* [C.32: If a class has a raw pointer (`T*`) or reference (`T&`), consider whether it might be owning / 如果一个类有原始指针(`T*`)或引用(`T&`)，考虑它是否可能是拥有的](#rc-dtor-ptr)
* [C.33: If a class has an owning pointer member, define a destructor / 如果一个类有拥有的指针成员，则定义析构函数](#rc-dtor-ptr2)
* [C.35: A base class destructor should be either public and virtual, or protected and non-virtual / 基类的析构函数应该是公共且虚函数，或者是受保护且非虚函数](#rc-dtor-virtual)
* [C.36: A destructor must not fail / 析构函数不能失败](#rc-dtor-fail)
* [C.37: Make destructors `noexcept` / 将析构函数声明为 `noexcept`](#rc-dtor-noexcept)

Constructor rules:

构造函数规则：

* [C.40: Define a constructor if a class has an invariant / 如果一个类有不变量，则定义构造函数](#rc-ctor)
* [C.41: A constructor should create a fully initialized object / 构造函数应创建一个完全初始化的对象](#rc-complete)
* [C.42: If a constructor cannot construct a valid object, throw an exception / 如果构造函数无法构造一个有效的对象，则抛出异常](#rc-throw)
* [C.43: Ensure that a copyable class has a default constructor / 确保可拷贝的类具有默认构造函数](#rc-default0)
* [C.44: Prefer default constructors to be simple and non-throwing / 倾向于默认构造函数简单且不抛出异常](#rc-default00)
* [C.45: Don't define a default constructor that only initializes data members; use member initializers instead / 不要定义仅初始化数据成员的默认构造函数；而是使用成员初始化器](#rc-default)
* [C.46: By default, declare single-argument constructors `explicit` / 默认情况下，将单参数构造函数声明为 `explicit`](#rc-explicit)
* [C.47: Define and initialize data members in the order of member declaration / 按照成员声明的顺序定义和初始化数据成员](#rc-order)
* [C.48: Prefer default member initializers to member initializers in constructors for constant initializers / 对于常量初始化器，在构造函数中优先使用默认成员初始化器而非成员初始化器](#rc-in-class-initializer)
* [C.49: Prefer initialization to assignment in constructors / 在构造函数中优先使用初始化而非赋值](#rc-initialize)
* [C.50: Use a factory function if you need "virtual behavior" during initialization / 如果在初始化期间需要“虚拟行为”，则使用工厂函数](#rc-factory)
* [C.51: Use delegating constructors to represent common actions for all constructors of a class / 使用委托构造函数来表示一个类的所有构造函数的共同操作](#rc-delegating)
* [C.52: Use inheriting constructors to import constructors into a derived class that does not need further explicit initialization / 使用继承构造函数将构造函数导入不需要进一步显式初始化的派生类](#rc-inheriting)

Copy and move rules:

拷贝和移动规则：

* [C.60: Make copy assignment non-`virtual`, take the parameter by `const&`, and return by non-`const&` / 使拷贝赋值操作非 `virtual`，参数通过 `const&` 传递，并通过非 `const&` 返回](#rc-copy-assignment)
* [C.61: A copy operation should copy / 拷贝操作应该进行拷贝](#rc-copy-semantic)
* [C.62: Make copy assignment safe for self-assignment / 使拷贝赋值对于自赋值是安全的](#rc-copy-self)
* [C.63: Make move assignment non-`virtual`, take the parameter by `&&`, and return by non-`const&` / 使移动赋值操作非 `virtual`，参数通过 `&&` 传递，并通过非 `const&` 返回](#rc-move-assignment)
* [C.64: A move operation should move and leave its source in a valid state / 移动操作应该移动并使源处于有效状态](#rc-move-semantic)
* [C.65: Make move assignment safe for self-assignment / 使移动赋值对于自赋值是安全的](#rc-move-self)
* [C.66: Make move operations `noexcept` / 使移动操作 `noexcept`](#rc-move-noexcept)
* [C.67: A polymorphic class should suppress public copy/move / 态类应该抑制公共的拷贝/移动](#rc-copy-virtual)

Other default operations rules:

其他默认操作规则：

* [C.80: Use `=default` if you have to be explicit about using the default semantics / 如果必须明确使用默认语义，请使用 `=default`](#rc-eqdefault)
* [C.81: Use `=delete` when you want to disable default behavior (without wanting an alternative) / 当想要禁用默认行为（且不希望有替代方案）时，使用 `=delete`](#rc-delete)
* [C.82: Don't call virtual functions in constructors and destructors / 不要在构造函数和析构函数中调用虚函数](#rc-ctor-virtual)
* [C.83: For value-like types, consider providing a `noexcept` swap function / 对于值类型，考虑提供一个 `noexcept` 的交换函数](#rc-swap)
* [C.84: A `swap` must not fail / `swap` 不能失败](#rc-swap-fail)
* [C.85: Make `swap` `noexcept` / 将 `swap` 声明为 `noexcept`](#rc-swap-noexcept)
* [C.86: Make `==` symmetric with respect of operand types and `noexcept` / 使 `==` 对于操作数类型是对称的，并声明为 `noexcept`](#rc-eq)
* [C.87: Beware of `==` on base classes / 注意基类上的 `==`](#rc-eq-base)
* [C.89: Make a `hash` `noexcept` / 将 `hash` 声明为 `noexcept`](#rc-hash)
* [C.90: Rely on constructors and assignment operators, not memset and memcpy / 依靠构造函数和赋值运算符，而不是 `memset` 和 `memcpy`](#rc-memset)

## <a name="ss-defop"></a>C.defop: Default Operations / 默认操作
...
### <a name="rc-zero"></a>C.20: If you can avoid defining default operations, do / 如果可以避免定义任何默认操作，那就避免
...
### <a name="rc-five"></a>C.21: If you define or `=delete` any copy, move, or destructor function, define or `=delete` them all / 如果定义或 `=delete` 任何拷贝、移动或析构函数，那么全部定义或 `=delete` 它们
...
### <a name="rc-matched"></a>C.22: Make default operations consistent / 使默认操作保持一致
...
## <a name="ss-dtor"></a>C.dtor: Destructors / 析构函数
...
### <a name="rc-dtor"></a>C.30: Define a destructor if a class needs an explicit action at object destruction / 如果一个类在对象销毁时需要明确的操作，则定义析构函数
...
### <a name="rc-dtor-release"></a>C.31: All resources acquired by a class must be released by the class's destructor / 一个类获取的所有资源都必须在该类的析构函数中释放
...
### <a name="rc-dtor-ptr"></a>C.32: If a class has a raw pointer (`T*`) or reference (`T&`), consider whether it might be owning / 如果一个类有原始指针(`T*`)或引用(`T&`)，考虑它是否可能是拥有的
...
### <a name="rc-dtor-ptr2"></a>C.33: If a class has an owning pointer member, define a destructor / 如果一个类有拥有的指针成员，则定义析构函数
...
### <a name="rc-dtor-virtual"></a>C.35: A base class destructor should be either public and virtual, or protected and non-virtual / 基类的析构函数应该是公共且虚函数，或者是受保护且非虚函数
...
### <a name="rc-dtor-fail"></a>C.36: A destructor must not fail / 析构函数不能失败
...
### <a name="rc-dtor-noexcept"></a>C.37: Make destructors `noexcept` / 将析构函数声明为 `noexcept`
...
## <a name="ss-ctor"></a>C.ctor: Constructors / 构造函数

A constructor defines how an object is initialized (constructed).

### <a name="rc-ctor"></a>C.40: Define a constructor if a class has an invariant / 如果一个类有不变量，则定义构造函数
...
### <a name="rc-complete"></a>C.41: A constructor should create a fully initialized object / 构造函数应创建一个完全初始化的对象
...
### <a name="rc-throw"></a>C.42: If a constructor cannot construct a valid object, throw an exception / 如果构造函数无法构造一个有效的对象，则抛出异常
...
### <a name="rc-default0"></a>C.43: Ensure that a copyable class has a default constructor / 确保可拷贝的类具有默认构造函数
...
### <a name="rc-default00"></a>C.44: Prefer default constructors to be simple and non-throwing / 倾向于默认构造函数简单且不抛出异常
...
### <a name="rc-default"></a>C.45: Don't define a default constructor that only initializes data members; use default member initializers instead / 不要定义仅初始化数据成员的默认构造函数；而是使用成员初始化器
...
### <a name="rc-explicit"></a>C.46: By default, declare single-argument constructors explicit / 默认情况下，将单参数构造函数声明为 `explicit`
...
### <a name="rc-order"></a>C.47: Define and initialize data members in the order of member declaration / 按照成员声明的顺序定义和初始化数据成员
...
### <a name="rc-in-class-initializer"></a>C.48: Prefer default member initializers to member initializers in constructors for constant initializers / 对于常量初始化器，在构造函数中优先使用默认成员初始化器而非成员初始化器
...
### <a name="rc-initialize"></a>C.49: Prefer initialization to assignment in constructors / 在构造函数中优先使用初始化而非赋值
...
### <a name="rc-factory"></a>C.50: Use a factory function if you need "virtual behavior" during initialization / 如果在初始化期间需要“虚拟行为”，则使用工厂函数
...
### <a name="rc-delegating"></a>C.51: Use delegating constructors to represent common actions for all constructors of a class / 使用委托构造函数来表示一个类的所有构造函数的共同操作
...
### <a name="rc-inheriting"></a>C.52: Use inheriting constructors to import constructors into a derived class that does not need further explicit initialization / 使用继承构造函数将构造函数导入不需要进一步显式初始化的派生类
...
## <a name="ss-copy"></a>C.copy: Copy and move / 拷贝和移动
...
### <a name="rc-copy-assignment"></a>C.60: Make copy assignment non-`virtual`, take the parameter by `const&`, and return by non-`const&` / 使拷贝赋值操作非 `virtual`，参数通过 `const&` 传递，并通过非 `const&` 返回
...
### <a name="rc-copy-semantic"></a>C.61: A copy operation should copy / 拷贝操作应该进行拷贝
...
### <a name="rc-copy-self"></a>C.62: Make copy assignment safe for self-assignment / 使拷贝赋值对于自赋值是安全的
...
### <a name="rc-move-assignment"></a>C.63: Make move assignment non-`virtual`, take the parameter by `&&`, and return by non-`const&` / 使移动赋值操作非 `virtual`，参数通过 `&&` 传递，并通过非 `const&` 返回
...
### <a name="rc-move-semantic"></a>C.64: A move operation should move and leave its source in a valid state / 移动操作应该移动并使源处于有效状态
...
### <a name="rc-move-self"></a>C.65: Make move assignment safe for self-assignment / 使移动赋值对于自赋值是安全的
...
### <a name="rc-move-noexcept"></a>C.66: Make move operations `noexcept` / 使移动操作 `noexcept`
...
### <a name="rc-copy-virtual"></a>C.67: A polymorphic class should suppress public copy/move / 多态类应该抑制公共的拷贝/移动
...
## C.other: Other default operation rules / 其他默认操作规则
...
### <a name="rc-eqdefault"></a>C.80: Use `=default` if you have to be explicit about using the default semantics / 如果必须明确使用默认语义，请使用 `=default`
...
### <a name="rc-ctor-virtual"></a>C.82: Don't call virtual functions in constructors and destructors / 不要在构造函数和析构函数中调用虚函数
...
### <a name="rc-swap"></a>C.83: For value-like types, consider providing a `noexcept` swap function / 对于值类型，考虑提供一个 `noexcept` 的交换函数
...
### <a name="rc-swap-fail"></a>C.84: A `swap` function must not fail / `swap` 不能失败
...
### <a name="rc-swap-noexcept"></a>C.85: Make `swap` `noexcept` / 将 `swap` 声明为 `noexcept`
...
### <a name="rc-eq"></a>C.86: Make `==` symmetric with respect to operand types and `noexcept` / 使 `==` 对于操作数类型是对称的，并声明为 `noexcept`
...
### <a name="rc-eq-base"></a>C.87: Beware of `==` on base classes / 注意基类上的 `==`
...
### <a name="rc-hash"></a>C.89: Make a `hash` `noexcept` / 将 `hash` 声明为 `noexcept`
...
* [C.100: Follow the STL when defining a container / 定义容器时遵循 STL 的做法](#rcon-stl)
* [C.101: Give a container value semantics / 为容器赋予值语义](#rcon-val)
* [C.102: Give a container move operations / 为容器提供移动操作](#rcon-move)
* [C.103: Give a container an initializer list constructor / 为容器提供初始化列表构造函数](#rcon-init)
* [C.104: Give a container a default constructor that sets it to empty / 为容器提供一个将其设置为空的默认构造函数](#rcon-empty)
* ???
* [C.109: If a resource handle has pointer semantics, provide `*` and `->` / 如果资源句柄具有指针语义，提供 `*` 和 `->`](#rcon-ptr)

**See also**: [Resources](#s-resource)


### <a name="rcon-stl"></a>C.100: Follow the STL when defining a container / 定义容器时遵循 STL 的做法
...
### <a name="rcon-val"></a>C.101: Give a container value semantics / 为容器赋予值语义
...
### <a name="rcon-move"></a>C.102: Give a container move operations / 为容器提供移动操作
...
### <a name="rcon-init"></a>C.103: Give a container an initializer list constructor / 为容器提供初始化列表构造函数
...
### <a name="rcon-empty"></a>C.104: Give a container a default constructor that sets it to empty / 为容器提供一个将其设置为空的默认构造函数
...
### <a name="rcon-ptr"></a>C.109: If a resource handle has pointer semantics, provide `*` and `->` / 如果资源句柄具有指针语义，提供 `*` 和 `->`
...
## <a name="ss-lambdas"></a>C.lambdas: Function objects and lambdas / 函数对象和 lambda 表达式
...
* [F.52: Prefer capturing by reference in lambdas that will be used locally, including passed to algorithms / 在本地使用的 lambda 表达式中，包括传递给算法时，优先通过引用捕获](#rf-reference-capture)
* [F.53: Avoid capturing by reference in lambdas that will be used non-locally, including returned, stored on the heap, or passed to another thread / 在非本地使用的 lambda 表达式中，包括返回、存储在堆上或传递给另一个线程时，避免通过引用捕获](#rf-value-capture)
* [ES.28: Use lambdas for complex initialization, especially of `const` variables / 使用 lambda 表达式进行复杂的初始化，尤其是对 `const` 变量的初始化](#res-lambda-init)

## <a name="ss-hier"></a>C.hier: Class hierarchies (OOP) / 类层次结构（面向对象编程）
...
* [C.120: Use class hierarchies to represent concepts with inherent hierarchical structure (only) / 仅当概念具有固有的层次结构时，使用类层次结构来表示](#rh-domain)
* [C.121: If a base class is used as an interface, make it a pure abstract class / 如果基类用作接口，请将其设为纯抽象类](#rh-abstract)
* [C.122: Use abstract classes as interfaces when complete separation of interface and implementation is needed / 当需要完全分离接口和实现时，使用抽象类作为接口](#rh-separation)

Designing rules for classes in a hierarchy summary:

* [C.126: An abstract class typically doesn't need a user-written constructor / 抽象类通常不需要用户编写的构造函数](#rh-abstract-ctor)
* [C.127: A class with a virtual function should have a virtual or protected destructor / 具有虚函数的类应该具有虚析构函数或受保护的析构函数](#rh-dtor)
* [C.128: Virtual functions should specify exactly one of `virtual`, `override`, or `final` / 虚函数应明确指定 `virtual`、`override` 或 `final` 中的一个](#rh-override)
* [C.129: When designing a class hierarchy, distinguish between implementation inheritance and interface inheritance / 在设计类层次结构时，区分实现继承和接口继承](#rh-kind)
* [C.130: For making deep copies of polymorphic classes prefer a virtual `clone` function instead of public copy construction/assignment / 对于多态类的深拷贝，优先使用虚 `clone` 函数，而非公共的拷贝构造/赋值](#rh-copy)
* [C.131: Avoid trivial getters and setters / 避免琐碎的获取器和设置器](#rh-get)
* [C.132: Don't make a function `virtual` without reason / 没有理由不要将函数设为虚函数](#rh-virtual)
* [C.133: Avoid `protected` data / 避免 `protected` 数据](#rh-protected)
* [C.134: Ensure all non-`const` data members have the same access level / 确保所有非 `const` 数据成员具有相同的访问级别](#rh-public)
* [C.135: Use multiple inheritance to represent multiple distinct interfaces / 使用多重继承来表示多个不同的接口](#rh-mi-interface)
* [C.136: Use multiple inheritance to represent the union of implementation attributes / 使用多重继承来表示实现属性的联合](#rh-mi-implementation)
* [C.137: Use `virtual` bases to avoid overly general base classes / 使用虚基类来避免过于通用的基类](#rh-vbase)
* [C.138: Create an overload set for a derived class and its bases with `using` / 使用 `using` 为派生类及其基类创建重载集合](#rh-using)
* [C.139: Use `final` on classes sparingly / 谨慎使用 `final` 修饰类](#rh-final)
* [C.140: Do not provide different default arguments for a virtual function and an overrider / 不要为虚函数和重写函数提供不同的默认参数](#rh-virtual-default-arg)

Accessing objects in a hierarchy rule summary:

* [C.145: Access polymorphic objects through pointers and references / 通过指针和引用访问多态对象](#rh-poly)
* [C.146: Use `dynamic_cast` where class hierarchy navigation is unavoidable / 在类层次结构导航不可避免时使用 `dynamic_cast`](#rh-dynamic_cast)
* [C.147: Use `dynamic_cast` to a reference type when failure to find the required class is considered an error / 当找不到所需的类被视为错误时，对引用类型使用 `dynamic_cast`](#rh-ref-cast)
* [C.148: Use `dynamic_cast` to a pointer type when failure to find the required class is considered a valid alternative / 当找不到所需的类被视为有效替代方案时，对指针类型使用 `dynamic_cast`](#rh-ptr-cast)
* [C.149: Use `unique_ptr` or `shared_ptr` to avoid forgetting to `delete` objects created using `new` / 使用 `unique_ptr` 或 `shared_ptr` 以避免忘记删除使用 `new` 创建的对象](#rh-smart)
* [C.150: Use `make_unique()` to construct objects owned by `unique_ptr`s / 使用 `make_unique()` 来构造由 `unique_ptr` 拥有的对象](#rh-make_unique)
* [C.151: Use `make_shared()` to construct objects owned by `shared_ptr`s / 使用 `make_shared()` 来构造由 `shared_ptr` 拥有的对象](#rh-make_shared)
* [C.152: Never assign a pointer to an array of derived class objects to a pointer to its base / 永远不要将派生类对象数组的指针赋给其基类的指针](#rh-array)
* [C.153: Prefer virtual function to casting / 优先使用虚函数而不是类型转换](#rh-use-virtual)

### <a name="rh-domain"></a>C.120: Use class hierarchies to represent concepts with inherent hierarchical structure (only) / 仅当概念具有固有的层次结构时，使用类层次结构来表示
...
### <a name="rh-abstract"></a>C.121: If a base class is used as an interface, make it a pure abstract class / 如果基类用作接口，请将其设为纯抽象类
...
### <a name="rh-separation"></a>C.122: Use abstract classes as interfaces when complete separation of interface and implementation is needed / 当需要完全分离接口和实现时，使用抽象类作为接口
...
## C.hierclass: Designing classes in a hierarchy: / 设计层次结构中的类：

### <a name="rh-abstract-ctor"></a>C.126: An abstract class typically doesn't need a user-written constructor / 抽象类通常不需要用户编写的构造函数
...
### <a name="rh-dtor"></a>C.127: A class with a virtual function should have a virtual or protected destructor / 具有虚函数的类应该具有虚析构函数或受保护的析构函数
...
### <a name="rh-override"></a>C.128: Virtual functions should specify exactly one of `virtual`, `override`, or `final` / 虚函数应明确指定 `virtual`、`override` 或 `final` 中的一个
...
### <a name="rh-kind"></a>C.129: When designing a class hierarchy, distinguish between implementation inheritance and interface inheritance / 在设计类层次结构时，区分实现继承和接口继承
...
### <a name="rh-copy"></a>C.130: For making deep copies of polymorphic classes prefer a virtual `clone` function instead of public copy construction/assignment / 对于多态类的深拷贝，优先使用虚 `clone` 函数，而非公共的拷贝构造/赋值
...
### <a name="rh-get"></a>C.131: Avoid trivial getters and setters / 避免琐碎的获取器和设置器
...
### <a name="rh-virtual"></a>C.132: Don't make a function `virtual` without reason / 没有理由不要将函数设为虚函数
...
### <a name="rh-protected"></a>C.133: Avoid `protected` data / 避免 `protected` 数据
...
### <a name="rh-public"></a>C.134: Ensure all non-`const` data members have the same access level / 确保所有非 `const` 数据成员具有相同的访问级别
...
### <a name="rh-mi-interface"></a>C.135: Use multiple inheritance to represent multiple distinct interfaces / 使用多重继承来表示多个不同的接口
...
### <a name="rh-mi-implementation"></a>C.136: Use multiple inheritance to represent the union of implementation attributes / 使用多重继承来表示实现属性的联合
...
### <a name="rh-vbase"></a>C.137: Use `virtual` bases to avoid overly general base classes / 使用虚基类来避免过于通用的基类
...
### <a name="rh-using"></a>C.138: Create an overload set for a derived class and its bases with `using` / 使用 `using` 为派生类及其基类创建重载集合
...
### <a name="rh-final"></a>C.139: Use `final` on classes sparingly / 谨慎使用 `final` 修饰类
...
### <a name="rh-virtual-default-arg"></a>C.140: Do not provide different default arguments for a virtual function and an overrider / 不要为虚函数和重写函数提供不同的默认参数
...
## C.hier-access: Accessing objects in a hierarchy / 在层次结构中访问对象

### <a name="rh-poly"></a>C.145: Access polymorphic objects through pointers and references / 通过指针和引用访问多态对象
...
### <a name="rh-dynamic_cast"></a>C.146: Use `dynamic_cast` where class hierarchy navigation is unavoidable / 在类层次结构导航不可避免时使用 `dynamic_cast`
...
### <a name="rh-ref-cast"></a>C.147: Use `dynamic_cast` to a reference type when failure to find the required class is considered an error / 当找不到所需的类被视为错误时，对引用类型使用 `dynamic_cast`
...
### <a name="rh-ptr-cast"></a>C.148: Use `dynamic_cast` to a pointer type when failure to find the required class is considered a valid alternative / 当找不到所需的类被视为有效替代方案时，对指针类型使用 `dynamic_cast`
...
### <a name="rh-smart"></a>C.149: Use `unique_ptr` or `shared_ptr` to avoid forgetting to `delete` objects created using `new` / 使用 `unique_ptr` 或 `shared_ptr` 以避免忘记删除使用 `new` 创建的对象
...
### <a name="rh-make_unique"></a>C.150: Use `make_unique()` to construct objects owned by `unique_ptr`s / 使用 `make_unique()` 来构造由 `unique_ptr` 拥有的对象

See [R.23](#rr-make_unique)

### <a name="rh-make_shared"></a>C.151: Use `make_shared()` to construct objects owned by `shared_ptr`s / 使用 `make_shared()` 来构造由 `shared_ptr` 拥有的对象

See [R.22](#rr-make_shared)

### <a name="rh-array"></a>C.152: Never assign a pointer to an array of derived class objects to a pointer to its base / 永远不要将派生类对象数组的指针赋给其基类的指针
...
### <a name="rh-use-virtual"></a>C.153: Prefer virtual function to casting / 优先使用虚函数而不是类型转换
...
## <a name="ss-overload"></a>C.over: Overloading and overloaded operators / 重载和重载运算符
...
* [C.160: Define operators primarily to mimic conventional usage / 定义运算符主要是为了模仿常规用法](#ro-conventional)
* [C.161: Use non-member functions for symmetric operators / 对于对称运算符使用非成员函数](#ro-symmetric)
* [C.162: Overload operations that are roughly equivalent / 重载大致等价的操作](#ro-equivalent)
* [C.163: Overload only for operations that are roughly equivalent / 仅为重载大致等价的操作](#ro-equivalent-2)
* [C.164: Avoid implicit conversion operators / 避免隐式转换运算符](#ro-conversion)
* [C.165: Use `using` for customization points / 对于定制点使用 `using`](#ro-custom)
* [C.166: Overload unary `&` only as part of a system of smart pointers and references / 仅将一元 `&` 重载作为智能指针和引用系统的一部分](#ro-address-of)
* [C.167: Use an operator for an operation with its conventional meaning / 对于具有常规含义的操作使用运算符](#ro-overload)
* [C.168: Define overloaded operators in the namespace of their operands / 在其操作数的命名空间中定义重载的运算符](#ro-namespace)
* [C.170: If you feel like overloading a lambda, use a generic lambda / 如果您想要重载 lambda，使用通用 lambda](#ro-lambda)

### <a name="ro-conventional"></a>C.160: Define operators primarily to mimic conventional usage / 定义运算符主要是为了模仿常规用法
...
### <a name="ro-symmetric"></a>C.161: Use non-member functions for symmetric operators / 对于对称运算符使用非成员函数
...
### <a name="ro-equivalent"></a>C.162: Overload operations that are roughly equivalent / 重载大致等价的操作
...
### <a name="ro-equivalent-2"></a>C.163: Overload only for operations that are roughly equivalent / 仅为重载大致等价的操作
...
### <a name="ro-conversion"></a>C.164: Avoid implicit conversion operators / 避免隐式转换运算符
...
### <a name="ro-custom"></a>C.165: Use `using` for customization points / 对于定制点使用 `using`
...
### <a name="ro-address-of"></a>C.166: Overload unary `&` only as part of a system of smart pointers and references / 仅将一元 `&` 重载作为智能指针和引用系统的一部分
...
### <a name="ro-overload"></a>C.167: Use an operator for an operation with its conventional meaning / 对于具有常规含义的操作使用运算符
...
### <a name="ro-namespace"></a>C.168: Define overloaded operators in the namespace of their operands / 在其操作数的命名空间中定义重载的运算符
...
### <a name="ro-lambda"></a>C.170: If you feel like overloading a lambda, use a generic lambda / 如果您想要重载 lambda，使用通用 lambda
...
## <a name="ss-union"></a>C.union: Unions / 联合类型
...
* [C.180: Use `union`s to save Memory / 使用 `union` 节省内存](#ru-union)
* [C.181: Avoid "naked" `union`s / 避免“裸” `union`](#ru-naked)
* [C.182: Use anonymous `union`s to implement tagged unions / 使用匿名 `union` 实现带标签的 `union`](#ru-anonymous)
* [C.183: Don't use a `union` for type punning / 不要使用 `union` 进行类型双关](#ru-pun)
* ???

### <a name="ru-union"></a>C.180: Use `union`s to save memory / 使用 `union` 节省内存
...
### <a name="ru-naked"></a>C.181: Avoid "naked" `union`s / 避免“裸” `union`
...
### <a name="ru-anonymous"></a>C.182: Use anonymous `union`s to implement tagged unions / 使用匿名 `union` 实现带标签的 `union`
...
### <a name="ru-pun"></a>C.183: Don't use a `union` for type punning / 不要使用 `union` 进行类型双关
...
# <a name="s-enum"></a>Enum: Enumerations / 枚举
...
* [Enum.1: Prefer enumerations over macros / 优先使用枚举而不是宏](#renum-macro)
* [Enum.2: Use enumerations to represent sets of related named constants / 使用枚举来表示相关的命名常量集合](#renum-set)
* [Enum.3: Prefer `enum class`es over "plain" `enum`s / 优先使用 `enum class` 而不是“普通”的 `enum`](#renum-class)
* [Enum.4: Define operations on enumerations for safe and simple use / 为枚举定义操作以确保安全和简单使用](#renum-oper)
* [Enum.5: Don't use `ALL_CAPS` for enumerators / 不要对枚举成员使用 `ALL_CAPS`（全大写）](#renum-caps)
* [Enum.6: Avoid unnamed enumerations / 避免未命名的枚举](#renum-unnamed)
* [Enum.7: Specify the underlying type of an enumeration only when necessary / 仅在必要时指定枚举的底层类型](#renum-underlying)
* [Enum.8: Specify enumerator values only when necessary / 仅在必要时指定枚举成员的值](#renum-value)

### <a name="renum-macro"></a>Enum.1: Prefer enumerations over macros / 优先使用枚举而不是宏
...
### <a name="renum-set"></a>Enum.2: Use enumerations to represent sets of related named constants / 使用枚举来表示相关的命名常量集合
...
### <a name="renum-class"></a>Enum.3: Prefer class enums over "plain" enums / 优先使用 `enum class` 而不是“普通”的 `enum`
...
### <a name="renum-oper"></a>Enum.4: Define operations on enumerations for safe and simple use / 为枚举定义操作以确保安全和简单使用
...
### <a name="renum-caps"></a>Enum.5: Don't use `ALL_CAPS` for enumerators / 不要对枚举成员使用 `ALL_CAPS`（全大写）
...
### <a name="renum-unnamed"></a>Enum.6: Avoid unnamed enumerations / 避免未命名的枚举
...
### <a name="renum-underlying"></a>Enum.7: Specify the underlying type of an enumeration only when necessary / 仅在必要时指定枚举的底层类型
...
### <a name="renum-value"></a>Enum.8: Specify enumerator values only when necessary / 仅在必要时指定枚举成员的值
...
# <a name="s-resource"></a>R: Resource management / 资源管理
...
* Resource management rule summary:
  ...
  * [R.1: Manage resources automatically using resource handles and RAII (Resource Acquisition Is Initialization) / 使用资源句柄和 RAII（资源获取即初始化）自动管理资源](#rr-raii)
  * [R.2: In interfaces, use raw pointers to denote individual objects (only) / 在接口中，仅使用原始指针来表示单个对象](#rr-use-ptr)
  * [R.3: A raw pointer (a `T*`) is non-owning / 原始指针（`T*`）是非拥有的](#rr-ptr)
  * [R.4: A raw reference (a `T&`) is non-owning / 原始引用（`T&`）是非拥有的](#rr-ref)
  * [R.5: Prefer scoped objects, don't heap-allocate unnecessarily / 优先使用作用域对象，不要不必要地进行堆分配](#rr-scoped)
  * [R.6: Avoid non-`const` global variables / 避免使用非 `const` 全局变量](#rr-global)

* Allocation and deallocation rule summary:

  * [R.10: Avoid `malloc()` and `free()` / 避免使用 `malloc()` 和 `free()`](#rr-mallocfree)
  * [R.11: Avoid calling `new` and `delete` explicitly / 避免显式调用 `new` 和 `delete`](#rr-newdelete)
  * [R.12: Immediately give the result of an explicit resource allocation to a manager object / 立即将显式资源分配的结果交给管理对象](#rr-immediate-alloc)
  * [R.13: Perform at most one explicit resource allocation in a single expression statement / 在单个表达式语句中最多执行一次显式资源分配](#rr-single-alloc)
  * [R.14: Avoid `[]` parameters, prefer `span` / 避免使用 `[]` 参数，优先使用 `span`](#rr-ap)
  * [R.15: Always overload matched allocation/deallocation pairs / 始终重载匹配的分配/释放对](#rr-pair)

* <a name="rr-summary-smartptrs"></a>Smart pointer rule summary:

  * [R.20: Use `unique_ptr` or `shared_ptr` to represent ownership / 使用 `unique_ptr` 或 `shared_ptr` 来表示所有权](#rr-owner)
  * [R.21: Prefer `unique_ptr` over `shared_ptr` unless you need to share ownership / 除非需要共享所有权，否则优先使用 `unique_ptr` 而非 `shared_ptr`](#rr-unique)
  * [R.22: Use `make_shared()` to make `shared_ptr`s / 使用 `make_shared()` 来创建 `shared_ptr`](#rr-make_shared)
  * [R.23: Use `make_unique()` to make `unique_ptr`s / 使用 `make_unique()` 来创建 `unique_ptr`](#rr-make_unique)
  * [R.24: Use `std::weak_ptr` to break cycles of `shared_ptr`s / 使用 `std::weak_ptr` 来打破 `shared_ptr` 的循环](#rr-weak_ptr)
  * [R.30: Take smart pointers as parameters only to explicitly express lifetime semantics / 仅将智能指针作为参数以明确表达生命周期语义](#rr-smartptrparam)
  * [R.31: If you have non-`std` smart pointers, follow the basic pattern from `std` / 如果您有非 `std` 的智能指针，请遵循 `std` 的基本模式](#rr-smart)
  * [R.32: Take a `unique_ptr<widget>` parameter to express that a function assumes ownership of a `widget` / 采用 `unique_ptr<widget>` 参数来表示函数承担 `widget` 的所有权](#rr-uniqueptrparam)
  * [R.33: Take a `unique_ptr<widget>&` parameter to express that a function reseats the `widget` / 采用 `unique_ptr<widget>&` 参数来表示函数重新设置 `widget`](#rr-reseat)
  * [R.34: Take a `shared_ptr<widget>` parameter to express shared ownership / 采用 `shared_ptr<widget>` 参数来表示共享所有权](#rr-sharedptrparam-owner)
  * [R.35: Take a `shared_ptr<widget>&` parameter to express that a function might reseat the shared pointer / 采用 `shared_ptr<widget>&` 参数来表示函数可能重新设置共享指针](#rr-sharedptrparam)
  * [R.36: Take a `const shared_ptr<widget>&` parameter to express that it might retain a reference count to the object ??? / 采用 `const shared_ptr<widget>&` 参数来表示它可能保留对对象的引用计数](#rr-sharedptrparam-const)
  * [R.37: Do not pass a pointer or reference obtained from an aliased smart pointer / 不要传递从别名智能指针获取的指针或引用](#rr-smartptrget)

### <a name="rr-raii"></a>R.1: Manage resources automatically using resource handles and RAII (Resource Acquisition Is Initialization) / 使用资源句柄和 RAII（资源获取即初始化）自动管理资源
...
### <a name="rr-use-ptr"></a>R.2: In interfaces, use raw pointers to denote individual objects (only) / 在接口中，仅使用原始指针来表示单个对象
...
### <a name="rr-ptr"></a>R.3: A raw pointer (a `T*`) is non-owning / 原始指针（`T*`）是非拥有的
...
### <a name="rr-scoped"></a>R.5: Prefer scoped objects, don't heap-allocate unnecessarily / 优先使用作用域对象，不要不必要地进行堆分配
...
### <a name="rr-global"></a>R.6: Avoid non-`const` global variables / 避免使用非 `const` 全局变量

See [I.2](#ri-global)

## <a name="ss-alloc"></a>R.alloc: Allocation and deallocation / 分配与释放

### <a name="rr-mallocfree"></a>R.10: Avoid `malloc()` and `free()` / 避免使用 `malloc()` 和 `free()`
...
### <a name="rr-newdelete"></a>R.11: Avoid calling `new` and `delete` explicitly / 避免显式调用 `new` 和 `delete`
...
### <a name="rr-single-alloc"></a>R.13: Perform at most one explicit resource allocation in a single expression statement / 在单个表达式语句中最多执行一次显式资源分配
...
### <a name="rr-ap"></a>R.14: Avoid `[]` parameters, prefer `span` / 避免使用 `[]` 参数，优先使用 `span`
...
### <a name="rr-pair"></a>R.15: Always overload matched allocation/deallocation pairs / 始终重载匹配的分配/释放对
...
## <a name="ss-smart"></a>R.smart: Smart pointers / 智能指针

### <a name="rr-owner"></a>R.20: Use `unique_ptr` or `shared_ptr` to represent ownership / 使用 `unique_ptr` 或 `shared_ptr` 来表示所有权
...
### <a name="rr-unique"></a>R.21: Prefer `unique_ptr` over `shared_ptr` unless you need to share ownership / 除非需要共享所有权，否则优先使用 `unique_ptr` 而非 `shared_ptr`
...
### <a name="rr-make_shared"></a>R.22: Use `make_shared()` to make `shared_ptr`s / 使用 `make_shared()` 来创建 `shared_ptr`
...
### <a name="rr-make_unique"></a>R.23: Use `make_unique()` to make `unique_ptr`s / 使用 `make_unique()` 来创建 `unique_ptr`
...
### <a name="rr-weak_ptr"></a>R.24: Use `std::weak_ptr` to break cycles of `shared_ptr`s / 使用 `std::weak_ptr` 来打破 `shared_ptr` 的循环
...
### <a name="rr-smartptrparam"></a>R.30: Take smart pointers as parameters only to explicitly express lifetime semantics / 仅将智能指针作为参数以明确表达生命周期语义

See [F.7](#rf-smart).

### <a name="rr-smart"></a>R.31: If you have non-`std` smart pointers, follow the basic pattern from `std` / 如果您有非 `std` 的智能指针，请遵循 `std` 的基本模式
...
### <a name="rr-uniqueptrparam"></a>R.32: Take a `unique_ptr<widget>` parameter to express that a function assumes ownership of a `widget` / 采用 `unique_ptr<widget>` 参数来表示函数承担 `widget` 的所有权
...
### <a name="rr-reseat"></a>R.33: Take a `unique_ptr<widget>&` parameter to express that a function reseats the `widget` / 采用 `unique_ptr<widget>&` 参数来表示函数重新设置 `widget`
...
### <a name="rr-sharedptrparam-owner"></a>R.34: Take a `shared_ptr<widget>` parameter to express shared ownership / 采用 `shared_ptr<widget>` 参数来表示共享所有权
...
### <a name="rr-sharedptrparam"></a>R.35: Take a `shared_ptr<widget>&` parameter to express that a function might reseat the shared pointer / 采用 `shared_ptr<widget>&` 参数来表示函数可能重新设置共享指针
...
### <a name="rr-sharedptrparam-const"></a>R.36: Take a `const shared_ptr<widget>&` parameter to express that it might retain a reference count to the object ??? / 采用 `const shared_ptr<widget>&` 参数来表示它可能保留对对象的引用计数 ？？？
...
### <a name="rr-smartptrget"></a>R.37: Do not pass a pointer or reference obtained from an aliased smart pointer / 不要传递从别名智能指针获取的指针或引用
...
# <a name="s-expr"></a>ES: Expressions and statements / 表达式和语句
...
# <a name="s-errors"></a>E: Error handling / 错误处理
...
# <a name="s-const"></a>Con: Constants and immutability / 常数和不变性
...
# <a name="s-templates"></a>T: Templates and generic programming / 模板与泛型编程
...
# <a name="s-cpl"></a>CPL: C-style programming / C风格编程
...
# <a name="s-source"></a>SF: Source files / 源文件
...
# <a name="s-stdlib"></a>SL: The Standard Library / 标准库
...
# <a name="s-a"></a>A: Architectural ideas / 架构理念
...
# <a name="s-not"></a>NR: Non-Rules and myths / 非规则和误区
...
# <a name="s-references"></a>RF: References / 参考文献
...
# <a name="s-profile"></a>Pro: Profiles / 配置文件
...
# <a name="S-gsl"></a>GSL: Guidelines support library / 指南支持库
...
# <a name="s-naming"></a>NL: Naming and layout suggestions / 命名和布局建议
...
# <a name="s-faq"></a>FAQ: Answers to frequently asked questions / 常见问题解答
...
# <a name="s-discussion"></a>Appendix C: Discussion / 附录 C: 讨论
...
# <a name="s-tools"></a>Appendix D: Supporting tools / 附录 D: 支持工具
...
# <a name="s-glossary"></a>Glossary / 词汇表
...
# <a name="s-unclassified"></a>To-do: Unclassified proto-rules / 待办事项: 未分类的原型规则
